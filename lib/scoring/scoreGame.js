...
import { unlockEligibleReferralsForWeek } from "../referrals/unlockEligibleReferralsForWeek";
...
export async function scoreGame(gameId) {
  ...
  // 2) Fetch predictions for this game
  ...
  const normalizePick = (v) => {
    ...
    if (s === "homewin" || s === "home" || s === "h") return "homeWin";
    if (s === "awaywin" || s === "away" || s === "a") return "awayWin";
    if (s === "draw" || s === "d" || s === "tie") return "draw";

    return s;
  };

  const computePoints = (pred, result) => {
    const p = normalizePick(pred);
    const r = normalizePick(result);
  ...
  // 8) Unlock eligible referrals after leaderboard is updated
  const referralResult = await unlockEligibleReferralsForWeek(weekStart);

  return {
    scored: preds.length,
    usersUpdated: perUser.size,
    weekStart,
    referralsUnlocked: referralResult?.unlocked || 0,
    referralWalletTxCreated: referralResult?.walletTxCreated || 0,
  };
}